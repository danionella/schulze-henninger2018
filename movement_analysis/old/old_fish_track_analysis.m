...
%% READ first chunk of frames and get background
...
%% analysis of spatial clustering (requires presence of 'out' structure with tracking data (see above)
...
%[h,p] = kstest2(nnd(1:250:end), nnd0(1:250:end));
...
for iFrame = 1:numel(out)
    ...
    vec0 = allvec(ceil(rand(size(vec, 1), 1)*size(allvec, 1)), :);
...
%% added by Tom on 170814: plotting orientation/direction analysis 

figure
polarhistogram(cat(1, thetas{:}),36*2,'FaceColor',[0 102 153]/255,'FaceAlpha',1) % Joerg's color
hold on
polarhistogram(cat(1, thetas0{:}),36*2,'displaystyle','stairs','edgecolor','k') %%'FaceColor',[.5 .5 .5],'FaceAlpha',.3);
hold off
ax = gca; ax.ThetaZeroLocation = 'top';
title(out(1).fn)
drawnow
saveas(gcf, [out(1).fn '_ori_RegNShuff.fig'])
saveas(gcf, [out(1).fn '_ori_RegNShuff.svg'])
